...
scen_to_load <- c(
  # "CAP_r32_sub_IDEEA_ELC_reg32_d365_h24_subset_1day_per_month_Y2025_2050_by_5",
  # "REF_r32_sub_IDEEA_ELC_reg32_d365_h24_subset_1day_per_month_Y2025_2050_by_5"
  "REF_r32_IDEEA_ELC_reg32_d365_h24_Y2050",
  # "REF_r32_IDEEA_ELC_reg32_d365_h24_Y2070",
  "CAP_r32_IDEEA_ELC_reg32_d365_h24_Y2050",
  # "CAP_r32_IDEEA_ELC_reg32_d365_h24_Y2070",
  # MH scenarios
  "REF_MH_v2_IDEEA_ELC_MH_d365_h24_Y2024_2050_by_5",
  "CAP_MH_v2_IDEEA_ELC_MH_d365_h24_Y2024_2050_by_5"
)
...
sns_r32 <- list(
  REF_2050 = .scen$REF_r32_2050,
  # REF_2070 = .scen$REF_r32,
  CAP_2050 = .scen$CAP_r32_2050
  # CAP_2070 = .scen$CAP_r32_2070
)

sns_MH <- list(
  REF_MH = .scen$REF_MH_v2,
  CAP_MH = .scen$CAP_MH_v2
)

sns_all <- c(sns_r32, sns_MH)

obj_test <- getData(sns_all, "vObjective", merge = T)
if (nrow(obj_test) != length(sns_all)) {
...
-   **CAP** - Carbon neutrality by 2050, linear decline.
...
pDemand <- getData(sns_r32[[1]], "pDemand", merge = T, process = T
                   # year = report_years
                   ) |>

# patch to plot figures
# load("data/new_load_tmp.RData")
# pDemand <- new_loadcurve |>
#   mutate(
#     scenario = "CAP25",
#     name = "pDemand",
#     process = "DEMELC_BY",
...
# pDemand <- pDemand |>
  ...
  mutate(
    datetime = tsl2dtm(slice, tmz = "Asia/Kolkata", year = 2070), 
...
```{r load-curve}
pDemand |>
  filter(year == min(year), process == "DEMELC_BY") |>
  ggplot() +
...
```{r}
load("data/load_curve_2025.RData")
# get demand data from IDEEA internal database
# elc_MH <- ideea_data$load_2019_MWh |>
#   filter(reg36 %in% "MH") |>
# group_by(reg36, MONTH, HOUR) |>
elc_MH <- load_curve_2025 |>
  filter(grepl("Maharashtra", State)) |>
  filter(year %in% min(year)) |> # comment this line to plot 2024-2025
  group_by(State, MONTH, HOUR) |>
  summarise(
...
## Total capacity, generation & storage (4h)
...
vTechCap_ELC <- getData(sns, name = "vTechCap", merge = T, 
  ...
  filter(!(process %in% c("ELCH2"))) |> # drop electrolyzers
...
<!-- ## Solar and Wind vs. (official) Potential, GW  -->
<!-- CAP scenario -->

```{r, eval=FALSE}
solar_wind_potential_r7 <- 
  fread("data/solar_wind_potential_r35_r7.csv.gz") |>
  group_by(reg7) |>
  summarise(
    ESOL = sum(`Solar Power Potential (in MW)`/1e3, na.rm = T),
    EWIN = sum(`Wind Power Potential (in MW)`/1e3, na.rm = T),
    .groups = "drop"
  ) |>
  rename(region = reg7) |>
  filter(!is.na(region), region != "") |>
  pivot_longer(cols = c(ESOL, EWIN), names_to = "process", 
               values_to = "Potential_GW")


vTechCap_SW <- getData(sns$CAP_2050, name = "vTechCap", merge = T, 
                        tech_ = "ESOL|EWIN|EWIF",
                        digits = 1, drop.zeros = T, process = T) |>
  drop_process_cluster() |> drop_process_vintage() |>
  ...
  left_join(solar_wind_potential_r7, by = c("region", "process"))
  
kableExtra::kable(
  vTechCap_SW |> 
    rename(Installed = value, Technology = process_label) |>
    rename(Potential = Potential_GW) |>
    select(region, Technology, Installed, Potential) |>
    arrange(region, Technology),
  digits = 1
)

```

<!-- ## Solar capacity vs. Potential (cont.) -->
<!-- (dots = Official estimates of potential) -->
```{r, eval=FALSE}
ggplot(vTechCap_SW) +
  geom_bar(aes(region, value, fill = process_label), stat = "identity") +
  geom_point(aes(region, Potential_GW, fill = process_label), 
             shape = 21, size = 5, colour = "yellow") +
  facet_grid(process_label~scenario) +
  scale_fill_viridis_d(option = "H", direction = -1, name = "Technology") +
  labs(x = "", y = "GW") +
  theme_bw()
...
## Solar installatons 2025-20** by region, GW
...
```{r}
all_slices <- sns_MH$CAP_MH@settings@calendar@timetable$slice

# (very) quick plots to use
# ?ideea_snapshot
ideea_snapshot2(sns_MH$REF_MH, YEAR = 2050, SLICE = all_slices[1:24])
...
```{r}
ideea_snapshot2(sns_MH$CAP_MH, YEAR = 2050, 
                # region = "MH",
                SLICE = all_slices[1:24])
ideea_snapshot2(sns$CAP_2070, YEAR = 2070, SLICE = all_slices[1:24])

getData(sns$CAP_2050, 
        name = c("vTechOut", "vStorageOut", 
                 "vStorageInp", "vTradeIr"), comm = "ELC",
        merge = T, )
...
gen_data <- ideea_snapshot2(
 ...
 gen_data |>
   mutate(
     tech_type = if_else(name == "vStorageInp", "Battery-In", tech_type),
     tech_type = if_else(name == "vStorageOut", "Battery-Out", tech_type)
   ) |>
   group_by(scenario, tech_type, year) |>
   summarise(value = sum(value), .groups = "keep") |>
   ggplot() +
   # geom_area(aes(datetime, value, fill = tech_type)) +
   geom_col(
     aes(as.factor(year), value / 1e3, fill = tech_type),
...
ideea_sol_cl_sf <- get_ideea_cl_sf("sol", nreg = nreg, tol = .01) 

vTechCap <- getData(sns_r32,
  name = "vTechCap", tech_ = "^E", process = T,
  merge = T, digits = 3, drop.zeros = T
)

vTechCap_MH <- getData(sns_MH,
  name = "vTechCap", tech_ = "^E", process = T,
  merge = T, digits = 3, drop.zeros = T
)
...
ggplot() +
  geom_sf(data = ideea_MH_sf) +
  geom_sf(aes(fill = GW), data = vTechCap_ESOL_MH_sf, color = NA, na.rm = T) +
  # scale_fill_viridis_c(option = "H") +
  scale_fill_viridis_c(option = "C") +
  facet_grid(scenario~year) +
  theme_ideea_map()
ggsave(filename = "images/solar_installations_by_cluster_MH.png", 
...
```{r}
ideea_win_cl_sf <- get_ideea_cl_sf("win", nreg = nreg, tol = .01) 
vTechCap_EWIN_sf <-
  vTechCap |>
  filter(year %in% report_years) |>  
  filter(region %in% "MH") |>
...
ggplot() +
  # geom_sf(data = ideea_sf) +
  geom_sf(data = ideea_MH_sf) +
  geom_sf(aes(fill = GW), data = vTechCap_EWIN_MH_sf, color = NA, na.rm = T) +
  scale_fill_viridis_c(option = "D") +
  # scale_fill_viridis_c(option = "D", transform = "log10") +
  facet_grid(scenario~year) +
  theme_ideea_map()
ggsave(filename = "images/wind_installations_by_cluster_MH.png", 
       width = 10, height = 5, scale = 1)
```
...
# getData(sns, c("pTradeStock", "vTradeCap"), process = T, merge = T, digits = 1, year = 2050) |> pivot_wider(names_from = name) |> as.data.table()
...
vTradeIr |>
  ...
  # select(-year) |>
  filter(grepl("MH", src) | grepl("MH", dst)) |>
  pivot_wider(names_from = scenario) |>
...
## Trade hourly profile
```{r}
# combine source and destination in one table
# electricity export (+)
src <- vTradeIr |> select(-dst) |> rename(region = src)

# electricity import (-)
dst <- vTradeIr |> select(-src) |> rename(region = dst) |> 
  mutate(value = -value)

net_trade <- rbind(src, dst) |>
  group_by(scenario, region, year, slice) |>
  summarise(value = sum(value, na.rm = T), .groups = "drop")

# add date-time
net_trade <- net_trade |>
  filter(year == 2070) |>
  filter(region %in% c("MH", "KA")) |>
  # filter(scenario %in% c("CAP"))
  mutate(
    datetime = tsl2dtm(slice, tmz = "Asia/Kolkata", year = 2050), 
    DATE = as.Date(datetime),
    HOUR = tsl2hour(slice),
    YDAY = tsl2yday(slice),
    MONTH = tsl2month(slice) |> factor(levels = 1:12, ordered = TRUE),
...
## Trade profile (heatmap), export (+) and import (+) by region
```{r}
net_trade |>
  filter(grepl("CAP", scenario)) |>
  ggplot() +
  geom_tile(aes(DATE, HOUR, fill = value)) +
  scale_fill_viridis_c(option = "H", name = "GWh") +
  scale_x_date(expand = c(0, 0)) +
...
```{r}
vExportRow <- getData(sns_MH, "vExportRow", comm = "ELC", 
                      merge = T, digits = 1) 
...
## Hydrogen  
```{r}
vTechCap_H2 <- getData(sns, name = "vTechCap", merge = T, tech = "ELCH2",
                           digits = 1, drop.zeros = T, process = T) |>
  # data pre-processing for figures:
  group_by(scenario, process, year) |>
  summarise(value = sum(value), .groups = "drop")
...
ggplot(vTechCap_H2) +
  geom_bar(aes(as.factor(year), value, fill = process), 
           stat = "identity") +
  facet_wrap(~scenario) +
  scale_fill_viridis_d(option = "H", direction = -1, name = "Technology") +
  # facet_grid(~region) +
